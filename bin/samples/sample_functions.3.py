...
my_base_name = Path(my_file_name).stem
...
def get_single_slide(presentation, slidetokeep):
    for idx, slide in enumerate(presentation.slides):
        if idx < slidetokeep:
            delete_slide(presentation, slide)
        elif (idx > slidetokeep):
            delete_slide(presentation, slide)
    presentation.save(SAVE_DIR + my_base_name + "-page-" + str(slidetokeep + 1) + ".pptx")
