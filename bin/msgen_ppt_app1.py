...
PARSER.add_argument('-t', metavar='<template>', dest='template', help='specify template')
PARSER.add_argument('-s', metavar='<vendordir>', dest='vendordir', help='specify vendor files')
PARSER.add_argument('-u', metavar='<clientdir>', dest='clientdir', help='specify client files')
...
CLIENTPATH = ARGS.clientdir
if os.path.isdir(CLIENTPATH):
    CLIENT_CSV = os.path.basename(glob.glob(os.path.join(os.path.abspath(CLIENTPATH), 'user.*'))[0])
    CLIENT_IMG = os.path.basename(glob.glob(os.path.join(os.path.abspath(CLIENTPATH), 'logo.*'))[0])
    sys.path.append(os.path.abspath(CLIENTPATH))
else:
    print(f'Path not accessible:: {CLIENTPATH}')
    sys.exit()

VENDORPATH = ARGS.vendordir
if os.path.isdir(VENDORPATH):
    VENDOR_CSV = os.path.basename(glob.glob(os.path.join(os.path.abspath(VENDORPATH), 'user.*'))[0])
    VENDOR_IMG = os.path.basename(glob.glob(os.path.join(os.path.abspath(VENDORPATH), 'logo.*'))[0])
    sys.path.append(os.path.abspath(VENDORPATH))
else:
    print(f'Path not accessible:: {VENDORPATH}')
    sys.exit()
...
add_front_logo(CLIENTPATH, CLIENT_IMG, 9, .5, 2)
...
add_logo(CLIENTPATH, CLIENT_IMG, 10.5, 1.5, 2)
add_support_table(CLIENTPATH, CLIENT_CSV, 1, 1.5, 9, .5)

add_logo(VENDORPATH, VENDOR_IMG, 10.5, 4, 2)
add_support_table(VENDORPATH, VENDOR_CSV, 1, 4, 9, .5)
